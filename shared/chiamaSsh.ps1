...
function get-sshText( [string] $p_Host, [string]$p_comando ) {
  ...
  Try {
        # $a = New-SshSession -ComputerName $p_Host -Username root -KeyFile $script:myKeyFile
        $username = "root"
        $pswd = ConvertTo-SecureString "mypswd" -AsPlainText -Force
        $credential = New-Object System.Management.Automation.PSCredential($username,$pswd)
        $sess = New-SshSession -ComputerName $p_Host -Credential $credential -KeyFile $script:myKeyFile
        $a = Invoke-SshCommand -SSHSession $sess -Command $p_comando  
        if ( $a.output -is [array] ) {
          foreach ( $sz in $a.output ) {
            $l_retfil += $sz +  "`n"
          }
        } else {
          $l_retfil += $a.output +  "`n"
        }
        log-debug ( "letto host: {0} con comando {1}" -f $p_Host, $p_comando )
        log-debug ( $a )
        $a = Remove-SshSession -SSHSession $sess
    } Catch {
        $script:inErrore = $true
        $outMsg = $_.Exception.Message
        log-err ( "Errore SSH su {0}`tcausa:{1}" -f $p_Host, $outMsg )
    }
...
# --------------------------------------------------------------------------
function chiama-ssh( [array]$p_arrHo, [string]$p_comando ) {

  $l_retfil = ""

ForEach ( $ho in $p_arrHo ) {
  Try {
        $a = New-SshSession -ComputerName $ho -Username root -KeyFile $script:myKeyFile
        $a = Invoke-SshCommand -ComputerName $ho -Command $p_comando -Quiet
        $l_retfil += $a
        $l_retfil += "`n"
        log-debug ( "letto host: {0} con comando {1}" -f $ho, $p_comando )
        log-debug ( $a )
        $a = Remove-SshSession -ComputerName $ho
    } Catch {
        $script:inErrore = $true
        $outMsg = $_.Exception.Message
        log-err ( "Errore SSH su {0}`tcausa:{1}" -f $ho, $outMsg )
    }
  }
  return $l_retfil
}
